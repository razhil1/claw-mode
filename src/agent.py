...
7. When building HTML/CSS/JS projects, always make them self-contained in a single file if possible.
...
- Always use consistent spacing, font hierarchy, and color palettes"""
...
def _execute_tool(tool_name: str, payload: str) -> str:
    ...
    try:
        if tool_name == "ListDirTool":
            ...
            return f"Unknown tool: {tool_name}. Available tools: ListDirTool, FileReadTool, ViewFileLinesTool, SearchTool, FileEditTool, FileDeleteTool, BashTool"
...
class ClawAgent:
    ...
    def clear_stop(self):
        """Clear the stop signal (call before starting a new run)."""
        self._stop_event.clear()

    def _build_messages(self, user_prompt: str) -> list[dict[str, str]]:
        msgs = [{"role": "system", "content": SYSTEM_PROMPT}]
        # Keep last 20 history items to stay within context limits
    ...
    def run_streaming(self, user_prompt: str) -> Generator[dict, None, None]:
        ...
        Event types:
          ...
          - error: error message
          - key_error: API key problem {error_type, message}
        ...
        for turn in range(MAX_AGENT_TURNS):
            ...
            if tool_call:
                ...
                # Emit clean display text (minus the TOOL line and thought)
                ...
                if tool_name == "FileEditTool":
                    parts_fe = payload.split("|||", 1)
                    if parts_fe:
                        files_changed.append(parts_fe[0].strip())

                yield {"type": "tool_result", "tool": tool_name, "result": result[:2000], "elapsed": elapsed}
        ...
        self.history.append({"role": "assistant", "content": combined_assistant[:5000]})
