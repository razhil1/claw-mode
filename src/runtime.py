...
class PortRuntime:
    ...
    def run_turn_loop(self, prompt: str, limit: int = 5, max_turns: int = 8, structured_output: bool = False) -> list[TurnResult]:
        engine = QueryEnginePort.from_workspace()
        ...
        # Full conversation maintained across the turn loop so every LLM call has context
        for turn in range(max_turns):
            result = engine.submit_message(current_prompt, command_names, tool_names, ())
            results.append(result)

            # Store what the assistant said in history so next turn is aware 
            engine.history_list.append({"role": "user", "content": current_prompt})
            engine.history_list.append({"role": "assistant", "content": result.output})

            raw_output = result.output

            # Parse TOOL: call from the last occurrence (in case of multi-line answer)
            tool_match = None
            ...
            if tool_match:
                try:
                    tool_part = tool_match[len("TOOL:"):].strip()
                    # Split on first | only
                    if " ||| " in tool_part:
                        tool_name, payload = tool_part.split(" ||| ", 1)
                    else:
                        tool_name, payload = tool_part.split("|", 1)
                    tool_name = tool_name.strip()
                    payload = payload.strip()

                    print(f"[Turn {turn+1}] Executing {tool_name}: {payload[:80]}...")
                    exec_result = execute_tool(tool_name, payload)

                    # Feed tool result back into next turn  
                    current_prompt = f"Tool result for {tool_name}:\n{exec_result.message}"
                except Exception as e:
                    current_prompt = f"Tool parsing error: {str(e)}. Please continue."
            else:
                # No tool call means agent finished naturally
                print(f"[Turn {turn+1}] Agent completed (no tool call).")
                break
